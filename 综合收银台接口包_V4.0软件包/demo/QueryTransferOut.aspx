...
<script runat="server">
    ...
    protected void btnButton_Click(object sender, EventArgs e)
    {
        ...
        //1、生成外转（出金）查询请求对象
        com.abc.trustpay.client.ebus.QueryTransferOut tRequest = new com.abc.trustpay.client.ebus.QueryTransferOut();
        tRequest.dicRequest["TransferNo"] = TransferNo.Text;   //外转（出金）交易编号 （必要信息）


        //2、传送外转（出金）查询请求并取得结果信息
        tRequest.postJSONRequest();

        //3、判断外转（出金）查询结果状态，进行后续操作
...
<head>
    <title>农行网上支付平台-商户接口范例-外转（出金）查询接口</title>
    <style type="text/css">
        .auto-style1
        {
            height: 55px;
